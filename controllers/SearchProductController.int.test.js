...
import app from '../server.js'; // Import your app here
...
} from "../config/testdb.js";  // Import the test DB utilities for in-memory DB

// No need to use Mongo URI from .env for in-memory database

let category;  // Declare category here to be used in cleanup

beforeAll(async () => {
  // Connect to the in-memory MongoDB instance (Test DB)
...
afterAll(async () => {
  ...
  await disconnectFromTestDb(); // Disconnect from the in-memory database
});
